...
class Planner:
    ...
    def compare_state(self,state1,state2, open_or_not):

        if abs(state1[0]-state2[0])+abs(state1[1]-state2[1])+abs(state1[2]-state2[2]) < 0.01: #arbitrarily small
            #print(state1, state2, "SAME", open_or_not)
            return True
        else:
            #print(abs(state1[0]-state2[0])+abs(state1[1]-state2[1])+abs(state1[2]-state2[2]))
            #print(state1, state2, "NOT SAME")
            return False
    ...
    def generate_plan(self):
        ...
        open_list_set.add(start_node.discretized_state)
        possible_v = [1.0]
        possible_w = [-pi,-3*pi/4,-pi/2, -pi/4, 0, pi/4, pi/2,3*pi/4, pi]
        while len(open_list) > 0:

            min_f_node = heapq.heappop(open_list)[1]
            open_list_set.remove(min_f_node.discretized_state)
            closed_list.add(min_f_node.discretized_state)
            #print("min_f state",min_f_node.discretized_state, min_f_node.f, min_f_node.g, min_f_node.h)#, min_f_node.past_actions)
            ...
            for v in possible_v:
                for w in possible_w:
                    move_forward = self.motion_predict(min_f_node.state[0],min_f_node.state[1],min_f_node.state[2],v,w)
                    if move_forward:
                        past_act_list = copy.deepcopy(min_f_node.past_actions)
                        past_act_list.append([v,w])
                        adj_nodes.append(Node(parent = min_f_node , state = move_forward, past_actions= past_act_list))
            #print("Size of adj_nodes",len(adj_nodes))
            for node in adj_nodes:
                #print(node.discretized_state)
                bool_add = True
                if node.discretized_state in closed_list:
                    bool_add = False
                    continue

                node.g = min_f_node.g + 0.5
                node.h = ((node.state[0] - self.goal.pose.position.x)**2 + (node.state[1] - self.goal.pose.position.y)**2)**0.5 #euclidean straightline dist without considering non-holonomic constraints
                node.f = node.g + node.h
                #print("Size of open list",len(open_list))
                if node.discretized_state in open_list_set:
                    bool_add = False
                    continue
                if bool_add:
                    heapq.heappush(open_list,(node.f,node))
                    open_list_set.add(node.discretized_state)
                    #print("Accepted")
                #else:
                    #print("Rejected")
...
if __name__ == "__main__":
    ...
    #radius of disc is (2*0.2552**2)**0.5/2 == 0.1804
    #therefore at 0.02 resolution, we set inflation pixels to be 10
    #at 0.05, we set inflation pixel to be 4
